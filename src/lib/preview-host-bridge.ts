 ...
 * Top-level runs (clone, local `npm run dev`, deployed sites) noop.
 */
 ...
 * top-level or under a parent that is not the preview host. Client-only;
 * null during SSR.
 */
 ...
 * Noops (returns a no-op dispose) when not embedded under the preview parent.
 */
